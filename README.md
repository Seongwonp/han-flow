...
현재 가장 큰 차이는 표 셀 내부의 여러 문단을 페이지 사이에서 나누지 못해 2·3페이지의 콘텐츠
분배가 달라지는 점입니다. 원문 글꼴이 없는 Mac에서는 대체 글꼴 폭에 따른 줄바꿈 차이도
있습니다. 함초롬체는 제3자 앱 재배포 권한이 확인되지 않아 번들하지 않고, 시스템 설치본의
한글·영문 family 이름을 찾아 사용합니다. OFL Noto Serif KR 번들도 실험했지만 페이지 분배가
개선되지 않아 채택하지 않았습니다. 자세한 결정은 [글꼴 전략](docs/font_strategy.md)을 참고하세요.
