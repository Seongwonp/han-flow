...
- [x] 행·열·병합·분할을 Windows package 리본과 저장·재개봉으로 자동 검증
...
2. [x] 기존 HANGUL font-face 재사용을 우선하는 글꼴 family 편집 기반
3. [x] 정렬·간격·들여쓰기 편집과 기존 탭·목록 구조 유지 기반
4. 여러 문단 표 cell, 행·열, 테두리·배경
   - [x] 병합되지 않은 일반 body cell의 여러 문단 text 편집
   - [x] cell 내부 문단 횡단 선택·분할·병합
   - [x] 일반 body cell의 격리된 테두리·배경 편집 기반
   - [x] 단순 직사각형 표의 행 추가·삭제와 selection 재배치
   - [x] 단순 직사각형 표의 열 추가·삭제
     - [x] 오른쪽 빈 열 추가와 반복 머리글 대응 cell 복제
     - [x] 현재 열 삭제 시 모든 direct row와 반복 머리글의 대응 cell 제거
     - [x] 삭제 후 `colCnt`·`colAddr`·표 전체 너비와 source anchor ordinal 재계산
     - [x] 중간/마지막 열 삭제 selection 재배치와 exact undo/redo·Save As 검증
...
- [ ] 고정 HWP matrix를 동일한 상위 corpus catalog ID에 연결
- [ ] 다단·각주·수식·목록과 머리말·꼬리말 variant 확장
- [ ] production DOM matrix를 manifest ID와 연결해 core 추정·실측 차이를 함께 보고
